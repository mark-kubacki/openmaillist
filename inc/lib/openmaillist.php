...
final class openmaillist
{
	...
	public function get_all_lists() {
		return $this->superior->get_all_lists();
	}
	...
	public function get_list($list_id) {
		if(is_numeric($list_id)) {
			$list = $this->superior->get_list($list_id);
			if(!$list === false) {
				return $list;
			} else {
	...
	public function get_thread($thread_id) {
		if(is_numeric($thread_id)) {
			$thread = $this->superior->get_thread($thread_id);
			if(!$thread === false) {
				return $thread;
			} else {
	...
	public function put_email(oml_list $list, oml_email $input) {
		...
		try {
			$myMsg	= $this->superior->get_message();
	...
	private function add_attachments_to_msg(oml_message $msg, oml_email $email) {
		if($email->has_attachments()) {
			...
			foreach($email->write_attachments_to_disk() as $filename) {
				$tmp	= $this->superior->create_attachment($filename, $storage_part.'/'.$filename);
	...
	public function put_message(oml_list $receiving_list, oml_message $msg) {
		...
		if(!$msg->write_to_db()) {
			$msg->remove_from_db();
			$this->superior->delete_empty_threads($theList->get_unique_value());
